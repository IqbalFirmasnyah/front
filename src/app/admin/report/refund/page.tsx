...
function RefundsReportContent() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const url = `${base}/reports/refunds${qs ? `?${qs}` : ''}`;
      ...
      if (!res.ok) {
        ...
        throw new Error(errText || 'Gagal memuat report refund.');
      }
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const qs = buildQS();
      const base = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/+$/, '');
      const url = `${base}/reports/refunds.pdf${qs ? `?${qs}` : ''}`;
  ...
  return (
    <AdminShell title="Laporan Refund">
      <div className="container mx-auto px-0">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            ...
            <select value={status} onChange={(e) => setStatus(e.target.value)} className="border rounded-md px-3 py-2 text-sm w-full md:w-auto">
              {STATUS.map((s) => (
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          <Card>
            ...
            <CardContent className="text-3xl font-bold">{loading ? '...' : data?.summary.totalPengajuan ?? 0}</CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Total Refund Final</CardTitle>
            </CardHeader>
            <CardContent className="text-3xl font-bold">{loading ? '...' : formatIDR(data?.summary.totalFinal || 0)}</CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-8">
          <Card>
            ...
            <CardContent className="flex flex-wrap gap-2">{loading ? '...' : statusBadges || <span className="text-sm text-muted-foreground">Tidak ada data</span>}</CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Distribusi Metode</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-2">{loading ? '...' : metodeBadges || <span className="text-sm text-muted-foreground">Tidak ada data</span>}</CardContent>
          </Card>
        </div>
          ...
          <CardContent className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading && (
                  <tr>
                    <td colSpan={10} className="py-6 text-center text-muted-foreground">Memuat data...</td>
                  </tr>
                )}
                {!loading && data?.rows?.length === 0 && (
                  <tr>
                    <td colSpan={10} className="py-6 text-center text-muted-foreground">Tidak ada data</td>
                  </tr>
                )}
