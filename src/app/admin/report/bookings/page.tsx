...
function statusTone(s: string) {
  ...
  if (key.includes('cancel') || key.includes('expired')) return 'bg-red-100 text-red-800';
  return 'bg-gray-100 text-gray-800';
}

const BOOKING_STATUS = [
  { value: '', label: 'all status' },
  { value: 'waiting', label: 'waiting approve admin' },
...
function BookingsReportContent() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const url = `${base}/reports/bookings${qs ? `?${qs}` : ''}`;
      ...
      if (!res.ok) {
        ...
        throw new Error(errText || 'Gagal memuat report.');
      }
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const qs = buildQS();
      const base = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/+$/, '');
      const url = `${base}/reports/bookings.pdf${qs ? `?${qs}` : ''}`;
      const res = await fetch(url, {
  ...
  const statusBadges = useMemo(() => {
    ...
    if (entries.length === 0) return <span className="text-sm text-muted-foreground">Tidak ada data</span>;
    return entries.map(([k, v]) => (
      <span key={k} className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${statusTone(k)}`}>
        {k.replace(/_/g, ' ')}: {v}
      </span>
    ));
  }, [data]);
  ...
  return (
    <AdminShell title="Laporan Booking">
      <div className="container mx-auto px-0 max-w-7xl">
          ...
          <div className="w-full md:w-auto">
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              ...
              <div className="flex gap-2 sm:justify-end">
                <Button onClick={applyFilter} variant="secondary" disabled={loading} className="whitespace-nowrap">
                  <Filter className="h-4 w-4 mr-2" /> Terapkan
                </Button>
                <Button onClick={resetFilter} variant="ghost" className="whitespace-nowrap">
                  Reset
                </Button>
                <Button onClick={handleDownloadPdf} className="text-black whitespace-nowrap" variant="ocean" disabled={downloading || loading}>
                  {downloading ? (
                    <>
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-8">
          <Card>
            ...
            <CardContent className="text-3xl font-bold">{loading ? '...' : data?.summary.totalBooking ?? 0}</CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
            ...
            <CardContent className="text-sm">
              {loading ? '...' : `${fmtDate(data?.summary.period.from)} — ${fmtDate(data?.summary.period.to)}`}
              <div className="mt-1 text-xs text-muted-foreground">Dibuat: {loading ? '...' : fmtDate(data?.summary.generatedAt)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent className="text-3xl font-bold">{loading ? '...' : formatIDR(data?.summary.totalEstimasi || 0)}</CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 mb-8">
          <Card>
            ...
            <CardContent className="flex flex-wrap gap-2 min-h-[40px]">{loading ? <div className="h-6 w-40 bg-muted rounded animate-pulse" /> : statusBadges}</CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Distribusi Produk</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-2 min-h-[40px]">{loading ? <div className="h-6 w-40 bg-muted rounded animate-pulse" /> : produkBadges}</CardContent>
          </Card>
        </div>
          ...
          <CardContent className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading && (
                  <tr>
                    <td colSpan={9} className="py-6 text-center text-muted-foreground">Memuat data...</td>
                  </tr>
                )}
                {!loading && data?.rows?.length === 0 && (
                  <tr>
                    <td colSpan={9} className="py-6 text-center text-muted-foreground">Tidak ada data</td>
                  </tr>
                )}
                {!loading && data?.rows?.map((r, i) => (
                  <tr key={r.kode + i} className="border-b">
                    ...
                    <td className="py-2 pr-4">
                      ...
                      <br />
                      <span className="text-xs text-muted-foreground">{r.kode}</span>
                    </td>
                    <td className="py-2 pr-4">
                      <div className="font-semibold">{r.produk}</div>
                      <div className="text-xs text-muted-foreground">{r.lokasiAtauTujuan}</div>
                    </td>
                    <td className="py-2 pr-4">
                    ...
                    <td className="py-2 pr-4">
                      {r.supir}
                      <br />
                      <span className="text-xs text-muted-foreground">{r.armada}</span>
                    </td>
                    <td className="py-2 pr-4">
                      {r.customer}
                      <br />
                    ...
                    <td className="py-2 pr-4">
                      <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusTone(r.status)}`}>{r.status.replace(/_/g, ' ')}</span>
                    </td>
                    <td className="py-2 text-right">{formatIDR(r.estimasi)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {!loading && data && (
              <div className="text-right text-sm">
                ...
                <span className="font-bold">{formatIDR(data.summary.totalEstimasi)}</span>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="md:hidden space-y-4">
          ...
          {!loading && data?.rows?.length === 0 && (
            <Card>
              <CardContent className="p-6 text-center text-muted-foreground">Tidak ada data untuk periode ini.</CardContent>
            </Card>
          )}
          {!loading && data?.rows?.map((r, i) => (
            <Card key={r.kode + i}>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs text-muted-foreground">{fmtDate(r.tanggalBooking)}</div>
                ...
                <div className="text-xs text-muted-foreground">
                  {fmtDate(r.tMulai)} — {fmtDate(r.tSelesai)} · {r.durasi} hari
                </div>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">{r.armada}</div>
                  </div>
                </div>
                <Separator />
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right font-bold">{formatIDR(r.estimasi)}</div>
                </div>
              </CardContent>
            </Card>
          ))}
