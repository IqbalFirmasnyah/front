...
/** Page: bungkus client child dengan Suspense */
...
function BookingsReportContent() {
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      a.href = url;
      const range = (from ? from : 'ALL') + '_' + (to ? to : 'ALL');
  ...
  const statusBadges = useMemo(() => {
    ...
    return entries.map(([k, v]) => (
      <span
        key={k}
        className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${statusTone(k)}`}
      >
        {k.replace(/_/g, ' ')}: {v}
      </span>
    ));
            ...
            <div className="w-full md:w-auto">
              <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                ...
                <div className="flex gap-2 sm:justify-end">
                  <Button onClick={applyFilter} variant="secondary" className="whitespace-nowrap">
          ...
          <div className="grid gap-4 md:grid-cols-3 mb-8">
            ...
            <Card>
              ...
              <CardContent className="text-sm">
                {loading
                  ? '...'
                  : `${fmtDate(data?.summary.period.from)} — ${fmtDate(data?.summary.period.to)}`}
