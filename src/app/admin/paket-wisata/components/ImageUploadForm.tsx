...
export default function ImageUploadForm({
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const totalNewFiles = selectedFiles.length + files.length;
    if (totalNewFiles > MAX_FILES) {
      setError(`Maksimal ${MAX_FILES} file per unggahan. Anda memilih ${files.length} file baru, total menjadi ${totalNewFiles}.`);
      e.target.value = '';
      return;
    }

    setSelectedFiles(prev => [...prev, ...files]);
    // Clear file input after selection to allow selecting the same file again later
    e.target.value = '';
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Panggil fungsi upload yang disediakan oleh parent
    await onUpload(selectedFiles);
    
    // Reset state setelah berhasil diupload
    setSelectedFiles([]);
  };
  
  // Memoize file previews to prevent unnecessary re-renders
  ...
  const allImages = [
    ...existingImages.map(name => ({
        name,
        url: `http://localhost:3001/public/travel-images/${name}`,
        isNew: false
    })),
    ...filePreviews
  ];
    ...
    <div className="bg-white shadow-xl rounded-xl p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-6 border-2 border-dashed border-indigo-300 rounded-lg bg-indigo-50">
        <label className="flex flex-col items-center justify-center cursor-pointer">
          ...
          <span className="text-sm text-gray-500">Maksimal {MAX_FILES} file. Format: JPG, PNG, dll.</span>
          <input
            type="file"
            multiple
            accept="image/*"
              ...
              <div key={index} className="relative group overflow-hidden rounded-lg shadow-md border-2 border-gray-200">
                <img
                  ...
                  onError={(e) => {
                    ...
                    (e.target as HTMLImageElement).src = `https://placehold.co/128x128/eeeeee/333333?text=Broken%20Image`;
                  }}
                />
