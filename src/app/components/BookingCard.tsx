...
import { CreditCard, CalendarDays, Car, User, RefreshCcw, Wallet, BadgeDollarSign } from "lucide-react";
...
type Booking = {
  ...
  // <- bidang refund (datang dari backend)
    ...
    <Card className="overflow-hidden border border-zinc-200 shadow-sm">
      <CardContent className="p-5 space-y-3">
        ...
        <div className="grid gap-2 text-sm text-zinc-700 sm:grid-cols-2">
          ...
          {refundStatus && refundFinal != null && (
            <div className="flex items-center gap-2">
              <BadgeDollarSign className="h-4 w-4 text-zinc-500" />
              <span>Refund: <strong>{formatMoney(refundFinal as any)}</strong></span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" onClick={() => onReschedule(booking)} className="inline-flex items-center gap-2">
